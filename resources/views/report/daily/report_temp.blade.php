  ...
  <tbody>
    ...
    @foreach ($master_data as $master)
      <tr>
        ...
        <td style="text-align:center;">{{$master->Aktivitas_Pemeliharaan}}</td>
        <td style="text-align:center;">{{$master->Standard}}</td>
        <td style="text-align:center;">{{$master->Time_Est}}</td>
        @for ($i=1; $i <= 31; $i++)
          @foreach ($data[$master->Code] as $key => $val)
            ...
            @endphp
          @endforeach
          <td class="ok-ng" style="text-align:center;">{{$namaMP}}</td>
        @endfor
      </tr>
      @php
        $no++;
