...
<div class="row">
  <div class="col">
    ...
    <div class="col float-right">
      {{-- <a class="btn btn-primary float-right" href="{{url('report/daily/report/'.$reports['id'])}}">
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table class="table table-bordered table-sm">
                <tr>
                  <td width="100%">Nama Mesin : SEAT ASSY 2W LINE {{$reports['mesin']}}</td>
                  {{-- <td width="20%" rowspan="5" class="text-center signature">
                    Approval 1 <br>
                    Leader<br><br><br><br><br>
                    @if ($reports['approval1'] != "")
                      <i>Approved By</i> {{$reports['approval1']}}<br>
                      <i>On</i> {{$reports['tglapprove1']}}
                    @elseif (auth()->user()->level == 2)
                      <a href="{{route('report.daily.approve1', $reports['id'])}}" class="btn btn-primary btn-sm" name="button">
                  ...
                  <td width="20%" rowspan="5" class="text-center signature">
                    ...
                    @elseif (auth()->user()->level == 3)
                      @if ($reports['approval1'] == "")
                        <i>Waiting Leader</i>
                      @else
                        <a href="{{route('report.daily.approve2', $reports['id'])}}" class="btn btn-primary btn-sm" name="button">
                        <i class="fas fa-fw fa-edit"></i> Setujui</a>
                      @endif
                    @endif
                  </td>
                  <td width="60%" rowspan="5" class="text-center signature">
                    Approval <br>
...
@section('js')
  ...
  $('#btn-print').click(function() {
    var id = $(this).data('id');
    printExternal(window.location.origin+'/maintenance-aski2/report/daily/report/'+id);
  })
