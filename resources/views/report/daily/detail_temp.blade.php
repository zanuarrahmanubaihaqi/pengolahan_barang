...
<div class="row">
  ...
  <div class="col float-right">
    ...
    {{-- <a class="btn btn-primary float-right" href="{{url('report/daily/report/'.$data['bulan'].'/'.$data['tahun'].'/'.$mesin)}}">Export to PDF</a> --}}
  </div>
</div>
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table class="table table-bordered table-sm">
                <thead>
                  <tr>
                    <td width="75%">Nama Mesin : {{$mesin}}</td>
                  </tr>
                  <tr>
                    <td>Proses : </td>
                  </tr>
        ...
        <div class="table-responsive">
          <table class="table table-bordered rotated-header table-sm" border="1" width="20%">
            <thead>
              ...
              <tr>
                <td colspan="16" style="font-size:12px;">Area : SEAT ASSY 2W</td>
            ...
            <tbody>
              ...
              @foreach ($master_data as $master)
                <tr>
                  <td>{{$no}}</td>
                  <td>{{$master->Aktivitas_Pemeliharaan}}</td>
                  <td>{{$master->Standard}}</td>
                  <td class="text-center">{{$master->Time_Est}}</td>
                  @for ($i=1; $i <= 31; $i++)
                    @foreach ($data[$master->Code] as $key => $val)
                      @php
                        if ($key == $i) {
                          $namaMP = $val;
                          break;
...
@section('js')
  ...
  $(document).ready(function() {
    $('#btn-print').click(function() {
      ...
      printExternal(window.location.origin+'/maintenance-aski2/report/daily/report/'+bulan+'/'+tahun+'/'+mesin);
