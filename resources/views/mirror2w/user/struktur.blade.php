          ...
          <table class="table" id="table_daily">
            ...
            <tbody>
              ...
              @foreach ($struktur as $data)
                <tr>
                  <td>{{$no}}</td>
                  <td>{{$data->teknisi != null ? $data->teknisi->name : ''}}</td>
                  <!-- <td>{{$data->leader != null ? $data->leader->name : ''}}</td>
                  <td>{{$data->foreman != null ? $data->foreman->name : ''}}</td> -->
                  <td>{{$data->supervisor != null ? $data->supervisor->name : ''}}</td>
                ...
                <div class="modal fade" id="edit_modal{{$data->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                  <div class="modal-dialog">
                    <div class="modal-content">
                      <form action="{{route('mirror2w-user_management.update_struktur', $data->id)}}" method="post">
                            ...
                            <select class="form-control" name="teknisi">
                              ...
                              @foreach ($users as $user)
                                @if ($user->level == 1)
                                  ...
                                  @endphp
                                  <option value="{{$user->id}}" {{ $isselect }}>{{$user->name}}</option>
                                @endif
                              @endforeach
                            </select>
                          </div>
...
<div class="modal fade" id="add_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <form action="{{route('mirror2w-user_management.store_struktur')}}" method="post">
        <div class="modal-header">
